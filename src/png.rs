...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> anyhow::Result<Png> {
        if bytes[0..8] != Png::STANDARD_HEADER {
            bail!("Not valid header");
        }
        let mut chunks: Vec<Chunk> = vec![];
        let mut offset: usize = 8;
        while offset < bytes.len() {
            let len: usize = u32::from_be_bytes(chunk::array_from_slice(&bytes[offset..offset + 4])) as usize;
            if offset + 4 + 4 + len + 4 > bytes.len() {
